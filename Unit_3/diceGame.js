/* 
    * Let's build a dice game! 
    ...
    - Build function to monitor/check our turn/action state and advance play state (can we roll? are we done? is the game not started?)
...
function playState(newDieState) {
  ...
  if (validDieState.includes(newDieState)) {
    ...
    if (stateOfDie === "roll") {
      ...
      if (numberOfRolls === 3) {
        console.log(
          `Hey! You've rolled ${numberOfRolls} times and need to score! This roll won't count.`
        );
      } else {
        ...
        // Write message
        console.log(
          `You rolled a ${dieRollValue}! Would you like to roll again or finalize the score?`
        );
    ...
    console.error(
      `Invalid move! Cannot ${newDieState} after ${validDieState}!`
    );
  }
}
