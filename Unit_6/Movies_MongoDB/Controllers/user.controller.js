...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    const user = new User({
      ...
      // We want to pass in the password provided by the user, said it 13 times, and add to db
      password: bcrypt.hashSync(red.body.password, 13)
    ...
    const token = jwt.sign({id: user_id}, SECRET, {expiresIn: "1day",
  });
...
// Login endpoint use the .post() method as well
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    //1. Capture data provided by user (request body), use object destructuring (easy to grab key/value)
